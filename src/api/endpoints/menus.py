...
@router.post("/")
def create_menu(response: Response, menu: MenusBody, session: Session = Depends(get_sync_session)):
...
@router.get("/{menu_id}")
def get_target_menu(response: Response, menu_id, session: Session = Depends(get_sync_session)):
...
def update_target_menu(menu_id, menu: MenusBody, session: Session = Depends(get_sync_session)):
    result = CRUDMenu.update_target_menu(menu_id, menu, session)
    return result


@router.delete("/{menu_id}")
def delete_target_menu(menu_id, session: Session = Depends(get_sync_session)):
