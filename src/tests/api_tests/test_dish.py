...
class TestDish:
    ...
    def test_read_dishes(self, client):
        response = client.get(f'/api/v1/menus/{pytest.menu_id}/submenus/{pytest.menu_id}/dishes')
        assert response.status_code == 200
        assert response.json() == []

    def test_create_dish(self, client, fixture_dish):
        response = client.post(f'/api/v1/menus/{pytest.menu_id}/submenus/{pytest.submenu_id}/dishes',
                               json=fixture_dish[0])
        assert response.status_code == 201
        pytest.dish_id = str(response.json()['id'])

    def test_update_dish(self, client, fixture_dish):
        response = client.patch(f'/api/v1/menus/{pytest.menu_id}/submenus/{pytest.submenu_id}/dishes/{pytest.dish_id}',
                                json=fixture_dish[1])
    ...
    def test_delete_dish(self, client):
        response = client.delete(f'/api/v1/menus/{pytest.menu_id}/submenus/{pytest.submenu_id}/dishes/{pytest.dish_id}')
